...
fn do_test(data: &[u8]) {
    let tx_result: Result<digibyte::blockdata::transaction::Transaction, _> = digibyte::consensus::encode::deserialize(data);
    match tx_result {
        Err(_) => {},
        Ok(mut tx) => {
            let ser = digibyte::consensus::encode::serialize(&tx);
            assert_eq!(&ser[..], data);
            let len = ser.len();
            ...
            let no_witness_len = digibyte::consensus::encode::serialize(&tx).len();
